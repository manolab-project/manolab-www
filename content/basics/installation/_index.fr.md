...
Les étapes suivantes sont là pour vous aider à installer Manolab. 


## Installation à partir des paquets binaires

Manolab est pour le moment fourni pour deux plateformes :
- Windows
- Linux (x86-64 et ARM)

Le logiciel a été développé sur ces plateformes en priorité, y compris pour la carte Raspberry PI.


## Installation à partir des sources

Vous pouvez compiler vous-même Manolab à partir de son code source. Manolab utilise un projet CMake, donc soit vous passez par un IDE supportant ce système de construction, soit vous construisez l'application en ligne de commandes.

### Récupération du code source

Le code source est sotcké sur un dépôt Git. Vous aurez besoin d'un client Git pour le récupérer mais vous pouvez aussi le récupérer via le site Github où il est hébergé, en récupérerant une archive du dépôt au format ZIP.

```
git clone --recursive https://github.com/manolab-project/manolab-studio
...
La procédure suivante est adaptée à un système Linux Debian/Ubuntu et dérivés. Il faudra probablement adapter certains noms de packages en fonction de la version de votre système.

```
sudo apt install build-essential cmake libsdl2-dev libudev-dev
```

### Linux (Fedora)

```
yum install cmake SDL2-devel g++ libudev-devel
```

### Compilation par la ligne de commandes
